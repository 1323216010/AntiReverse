...
def generate_device_id():
    ...
    unique_id = get_disk_serial_number()
...
def get_disk_serial_number():
    try:
        if subprocess.os.name == 'nt':
            ...
            # Unix-like systems, for example, Linux
            output = subprocess.check_output("sudo hdparm -I /dev/sda | grep Serial", shell=True)
        serial_number = output.decode().strip().split()[-1]
